...
The supported runtime path is training. Validation loss can be enabled
explicitly with:
...
This flag only enables validation/eval loss; it does not make generation,
inference, or fused TP inference residual scaling supported.
...
YAML configs that omit `allow_depth_mup_eval` default it to `False`, matching the
CLI default.
