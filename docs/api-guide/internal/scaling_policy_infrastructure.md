...
resolved training policy. For `mup`, the policy preserves the existing width-MuP
rules:
...
- YAML namespaces should receive defaults for new global runtime flags, including
  `allow_depth_mup_eval=False`.
- Checkpoint compatibility should compare effective resolved scaling contexts
